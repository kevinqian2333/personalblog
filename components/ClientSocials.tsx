      ...
      {socials.map((social) => {
        ...
        const button = (
          <div
            ...
            title={social.label}
            onClick={social.isLink ? undefined : () => copyToClipboard(social.value!)}
          >
            {getIcon(social.type)}
          </div>
        );

        return social.isLink ? (
          <a
            key={social.type}
            href={social.value}
            target="_blank"
          ...
          <div key={social.type} className="relative group/social">
            {button}
            <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 dark:bg-slate-700 text-white text-xs px-2 py-1 rounded opacity-0 group-hover/social:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
              点击复制 {social.label}
            </span>
          </div>
        );
